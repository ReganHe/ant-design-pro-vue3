<template>
  ...
  <div class="footer" v-if="commands &&
    commands.length &&
    commands.some(
    ...
    <div class="commands">
      <Button v-for="(item, index) of commands" :key="index"
        v-show="!item.visibleValidator || item.visibleValidator({ model, operateType })" class="command"
        :loading="item.loading" :disabled="item.disableValidator && item.disableValidator.call({ model, operateType })"
        v-bind="item.elementProps" @click="handleEmit(item.command, { command: item })">{{ item.text }}
      </Button>
    </div>
  </div>
</template>
