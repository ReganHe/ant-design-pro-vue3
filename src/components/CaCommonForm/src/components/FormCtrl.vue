    ...
    extendProps.subTitle
      ...
      <div v-if="extendProps.groupCommands" :style="{ float: 'right' }">
        <Button v-for="(item, index) of extendProps.groupCommands"
          v-show="!item.visibleValidator || item.visibleValidator({ model, operateType })" :key="index" type="link"
          :disabled="item.disableValidator && item.disableValidator({ model, operateType })" v-bind="item.elementProps"
          @click.stop="handleEmit(item.command)">{{ item.text }}
        </Button>
      </div>
    </div>
