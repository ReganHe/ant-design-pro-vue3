<template>
  <Form ref="formRef" :model="model" layout="inline" v-bind="elementProps">
    <QueryCtrl v-for="(field, index) in fields" :key="index" :default-model="model" v-bind="field"
      @enterKeyDown="handleKeyDown" />
    <FormItem>
      <Button v-for="(item, index) of commands" v-show="!item.visibleValidator || item.visibleValidator()" :key="index"
        class="command" :disabled="item.disableValidator && item.disableValidator()" v-bind="item.elementProps"
