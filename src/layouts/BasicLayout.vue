<template>
  <a-layout :class="['layout', appStore.device]">
    ...
    <a-drawer
      v-if="isMobile"
      placement="left"
      :class="`drawer-sider ${settingsStore.theme}`"
      :closable="false"
      :open="collapsedRef"
      @close="drawerClose"
      width="256px"
    ...
    <side-menu
      v-else-if="settingsStore.layout !== 'topmenu'"
      mode="inline"
      :menus="permissionStore.routes"
      :theme="settingsStore.theme"
    ...
    />
    <a-layout
      :class="[settingsStore.layout, `content-width-${settingsStore.contentWidth}`]"
      :style="{
        paddingLeft: contentPaddingLeftRef,
        minHeight: '100vh'
      }"
    >
      <!-- layout header -->
      <global-header
        :mode="settingsStore.layout"
        :menus="permissionStore.routes"
        :theme="settingsStore.theme"
        :collapsed="collapsedRef"
        @toggle="toggle"
